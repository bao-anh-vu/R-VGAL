...
## Flags
date <- "20231018" #"20231121" has 2 fixed effects, ""20231030" has 4    
reload_data <- F
rerun_rvga <- T
rerun_stan <- F
save_data <- F
...
if (use_tempering) {
  n_obs_to_temper <- 10
  K <- 50
  a_vals_temper <- rep(1/K, K) # adapt this so that the number of tempering steps decreases until 1?
}
...
if (reload_data) {
  ...
  economic_ind <- read.csv(file = "./data/Poverty_Indicators_by_Community_Area.csv")
  ...
  # crime_pop_df <- left_join(crimedata_grouped_df, pop_data_sub, 
  #                     by = join_by(community_area == GEOID))
  # all_df <- left_join(crime_pop_df, economic_ind, 
  #                     by = join_by(community_area == Community.Area))
  
  if (save_data) {
    saveRDS(all_df, file = "./data/chicago_crime_all.rds")
  }
   
} else {
  all_df <- readRDS(file = "./data/chicago_crime_all.rds")
}
...
all_df <- all_df %>% mutate(year_centred = year - 2011, 
                            pop_scaled = X2010_POP/scale_factor,
                            log_pop_scaled = log(X2010_POP/scale_factor),
                            ncases_scaled = round(ncases/scale_factor)) #, 
                            # below_poverty = Below.Poverty.Level/100, 
                            # no_highschool = No.High.School.Diploma/100, 
                            # unemployment_rate = Unemployment/100)

standard_df <- all_df %>% ungroup() %>% 
  mutate(pop_std = (log_pop_scaled - mean(log_pop_scaled)) / sd(log_pop_scaled), 
...
library(lme4)
glm_fit <- glmer(ncases_scaled ~ 1 + year_std + pop_std + #below_poverty + 
             (1 + year_std | community_area),
      family = "poisson", data = standard_df)

# glm_fit2 <- glmer(ncases_scaled ~ 1 + year_std + pop_std  + (1 + year_std | community_area),
# family = "poisson", data = standard_df)
...
# test_comm_area = 40
# new_df <- data.frame(year_std = standard_df$year_std[1:19], community_area = test_comm_area,
#                      pop_std = filter(standard_df, community_area == test_comm_area)$pop_std[1])
#                      # log_pop = filter(all_df, community_area == test_comm_area)$log_pop[1])
# prediction <- predict(glm_fit, new_df, type = "response")
# plot(-9:9, prediction)
# lines(-9:9, filter(standard_df, community_area == test_comm_area)$ncases)
# browser()
#############################################################################

## Fixed covariate matrix
fixed_effects <- c("community_area", "year_std", "pop_std") #, 
                      # "below_poverty") #, "unemployment_rate")
X_long <- cbind(rep(1, nrow(standard_df)), standard_df[fixed_effects])
...
## Response variable
y_long <- standard_df[c("community_area", "ncases_scaled")]
...
fixed_efs <- c("0", "year", "population")
fixed_ef_names <- sapply(1:n_fixed_effects, function(x) paste0("beta[", fixed_efs[x], "]"))
param_names <- c(fixed_ef_names, "sigma_alpha[11]", "sigma_alpha[21]", "sigma_alpha[22]")  
...
fixed_vars <- lapply(1:n_fixed_effects, function(x) textGrob(bquote(beta[.(fixed_efs[x])])))
