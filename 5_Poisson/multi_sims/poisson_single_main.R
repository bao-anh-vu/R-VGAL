...
sim <- 94 #57 
poisson_data <- readRDS(file = paste0("./multi_sims/data/", date, 
...
if (reorder_data) {
  ...
  # which(reordered_ind == 161)
  # reordered_ind <- c(161, 1:160, 162:length(y))
  # reordered_ind <- c(1:9, 161, 10:160, 162:length(y))
...
hist(unlist(y), main = "Histogram of the data")
## Try removing the "outlier" here
if (remove_outlier) {
  y_new <- lapply(y, function(y_i) ifelse(y_i < 500, y_i, 100))
  y <- y_new
}
...
if (plot_prior) {
  ...
  s11 <- sort(rvgal.prior_samples[, 3])
  s21 <- sort(rvgal.prior_samples[, 4])
  s22 <- sort(rvgal.prior_samples[, 5])
  print(c(s11[2500], s11[97500]))
  print(c(s22[2500], s22[97500]))
  print(c(s21[2500], s21[97500]))
  
  par(mfrow = c(2,3))
  true_vals <- c(beta, c(Sigma_alpha[t(lower.tri(Sigma_alpha, diag = T))]))
  for (p in 1:param_dim) {
    plot(density(rvgal.prior_samples[, p]), main = paste0("Prior of ", param_names[p]))
...
if (rerun_rvgal) {
  ...
  results_file <- paste0("poisson_rvgal", temper_info, reorder_info,
...
if (rerun_stan) {
  ...
  hmc_results <- readRDS(file = hmc.result_file)
}
